  ...
  return (
    <SafeAreaView style={[styles.container, { backgroundColor: ui.background }]} edges={['top', 'bottom']}>
      <View style={styles.header}>
        ...
        <Text style={[styles.headerTitle, { color: ui.text }]}>Calculadora de promedio</Text>
        <TouchableOpacity>
      ...
      <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={styles.scrollContent} keyboardShouldPersistTaps="handled">
        <View style={styles.tableHeader}>
          <Text style={[styles.columnLabel, { flex: 2.5, color: ui.textMuted }]}>Asignatura</Text>
          <Text style={[styles.columnLabel, { flex: 1, textAlign: 'center', color: ui.textMuted }]}>Nota</Text>
          <Text style={[styles.columnLabel, { flex: 1, textAlign: 'center', color: ui.textMuted }]}>Créditos</Text>
          <View style={{ width: 30 }} />
        </View>
        ...
        <View style={styles.subjectsList}>
          {subjects.map((item) => (
            <View key={item.id} style={styles.subjectRow}>
              <TextInput
                style={[styles.subjectInput, { flex: 2.5, backgroundColor: ui.surfaceAlt, color: ui.text, borderColor: ui.border }]}
                value={item.name}
              ...
              />
              <TextInput
                style={[styles.gradeInput, { flex: 1, backgroundColor: ui.surfaceAlt, color: ui.text, borderColor: ui.border }]}
                value={item.grade}
              ...
              />
              <TextInput
                style={[styles.creditInput, { flex: 1, backgroundColor: ui.surfaceAlt, color: ui.text, borderColor: ui.border }]}
                value={item.credits}
        ...
        <TouchableOpacity style={[styles.addBtn, { borderColor: ui.border }]} onPress={addSubject}>
          <Ionicons name="add" size={20} color={ui.tint} />
          <Text style={[styles.addBtnText, { color: ui.tint }]}>Agregar asignatura</Text>
        </TouchableOpacity>

        <View style={[styles.resultCard, { backgroundColor: ui.surface, borderColor: ui.border }]}> 
          <View style={styles.resultHeader}>
            <Text style={[styles.resultTitle, { color: ui.text }]}>Promedio ponderado</Text>
            <Ionicons name="refresh-circle-outline" size={20} color={ui.tint} />
          </View>
          <Text style={[styles.resultValue, { color: ui.text }]}>{weightedAvg.toFixed(2)}</Text>
          <Text style={[styles.resultSubtext, { color: ui.textMuted }]}>Créditos totales: {totalCredits}</Text>
        </View>
      </ScrollView>
    </SafeAreaView>
  );
